...
export class CaptchaService {
  ...
  async solveCaptcha(page: Page, captchaSelector: string): Promise<boolean> {
    try {
      ...
      const success = await page.waitForFunction(
        () => !document.querySelector('#captcha-error'),
        { timeout: 5000 }
  ...
  async bypassRecaptcha(page: Page): Promise<boolean> {
    try {
      ...
      // 이미지 챌린지가 나타나는지 확인
      const hasChallenge = await page.waitForFunction(
        () => {
      ...
      this.logger.warn('reCAPTCHA challenge detected, using external service would be required');
